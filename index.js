...
module.exports.list = function (keys, architecture, callback) {
	...
	if (typeof callback === 'function') {
		...
		cscript.init(function (err) {
			...
			let args = baseCommand('regListStream.wsf', architecture);

			let child = execFile(cscript.path(), args, {encoding: 'utf8'}, function (err) {
				if (err) {
...
module.exports.createKey = function (keys, architecture, callback) {
	...
	let args = baseCommand('regCreateKey.wsf', architecture);
...
module.exports.deleteKey = function (keys, architecture, callback) {
	...
	let args = baseCommand('regDeleteKey.wsf', architecture);
...
module.exports.putValue = function (map, architecture, callback) {
	...
	let args = baseCommand('regPutValue.wsf', architecture);
...
module.exports.setBasePath = function(value) {
	basePath = value;
};
...
function baseCommand(cmd, arch) {
	return ['//Nologo', path.join(getBasePath(), 'vbs', cmd), arch];
}
